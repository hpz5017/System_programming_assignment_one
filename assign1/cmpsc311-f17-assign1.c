...
int main(int argc, char *argv[]) {
    ...
    printArray(array,0,SIZE_PROBLEM1-1);
    puts("");
   // printf("\nProblem 1 End");
    ...
    for (unsigned int i=0;strArray[i]!='\0';i++) 
    {
       printf("%c",strArray[i]); 
    }
    ...
    for (unsigned int i = 0; i < SIZE_PROBLEM3; i++) 
    {
        array_p3[i] = 1 + rand() % 500;
    }
    ...
    puts("");
    int smallest = recursiveMinimum(array_p3, 0 ,SIZE_PROBLEM3-1);
    ...
    if (1) //Change to 1 to complete this section
    {
        ...
        FILE *f;
        int array_p4[1024];
        //varible used to store number read from the file
        int number;
        //varible used to count amount of numbers in the file
        int amount=0;
        f=fopen("problem4_data.txt","r");
        while(fscanf(f,"%d,",&number)>0)
        {
            array_p4[amount]=number;
            amount++;
        }
        double mean_result = mean(array_p4,amount); // process mean
        double median_result = median(array_p4,amount); // process median
        if (mean_result == 6.6200)
        {
            if (median_result == 7.0)
            {
